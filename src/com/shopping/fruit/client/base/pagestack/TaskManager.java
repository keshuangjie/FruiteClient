...
public interface TaskManager {
	 ...
	 */
	String ACTION_NAVIGATE_PAGE = "act_navigate_page";
	String ACTION_NAVIGATE_BACK = "act_back";
	 ...
	 * 页面 URI scheme
	 */
	String PAGE_SCHEME = "page_scheme_uri";
	 ...
	 *            目标Tak的类名(全限定名)
	 */
	abstract void registerRootTask(String clsName);
	...
	/**
	 * 指定页面Tag跳转页面 </p> 为跳转的页面添加标签，构造多实例页面
	 * 
	 * @param ctx
	 *            Context
	 * @param pageClsName
	 ...
	 */
	abstract void navigateTo(Context ctx, String pageClsName,
                             String pageTagString);

	/**
	 * 指定页面Tag跳转页面，附加页面参数 </p> 为跳转的页面添加标签，构造多实例页面
	 * 
	 * @param ctx
	 *            Context
	 * @param pageClsName
	 ...
	 */
	abstract void navigateTo(Context ctx, String pageClsName,
                             String pageTagString, Bundle pageArgs);
	...
	/**
	 * 重置当前页面栈 </p> 清空当前页面栈，并将 record作为新的根页面
	 */
	...
	/**
	 * ClearTop操作 </p> 清除栈中record之后进栈的页面
	 */
	...
	abstract void attach(Activity activity);

	abstract void detach();

	abstract Activity getContainerActivity();
